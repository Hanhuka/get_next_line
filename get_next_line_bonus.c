...
/*   Updated: 2021/12/17 15:49:14 by ralves-g         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	two = bruh(fd, buf[fd]);
	free(buf[fd]);
	buf[fd] = cpyfrm2(two.buf, 0, -1);
	free(two.buf);
	if (buf[fd] == NULL && two.i == ft_strlen(two.changer))
		return (two.changer);
	two.changer = cpyfrm2(buf[fd], two.i + 1, -1);
	two.buf = cpyfrm2(buf[fd], 0, two.i);
	free(buf[fd]);
	if (two.changer != NULL)
		buf[fd] = cpyfrm2(two.changer, 0, -1);
	free(two.changer);
	return (two.buf);
}
